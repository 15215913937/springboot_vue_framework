    ...
    <div style="flex: 1">
      <div style="flex: 1;display: flex;max-height:400px">
        <el-card style="width:90%">
          ...
          <div style="margin: 10px;height: 120px;display: flex;flex-direction: column">
            <div style="flex:1;display: flex;margin: 20px 0;border: 1px solid black;border-radius: 5px">
              <div style="flex: 1;padding-left: 20px;font-size: 20px">本月支出:&nbsp&nbsp&nbsp-<span style="color: green">{{
                  currentMonthExpense
                }}</span>元
              </div>
              <div style="flex: 1;padding-left: 20px;font-size: 20px">本月收入:&nbsp&nbsp&nbsp+<span style="color: red">{{
                  currentMonthIncome
                }}</span>元
              </div>
            </div>
            <div style="flex:1;display: flex;margin: 20px 0;border: 1px solid black;border-radius: 5px">
              <div style="flex: 1;padding-left: 20px;font-size: 20px">上月支出:&nbsp&nbsp&nbsp-{{ lastMonthExpense }}元</div>
              <div style="flex: 1;padding-left: 20px;font-size: 20px">上月收入:&nbsp&nbsp&nbsp+{{ lastMonthIncome }}元</div>
            </div>
          </div>
        </el-card>
      </div>
...
export default {
  ...
  mounted() {  //页面元素渲染完成后再触发mounted
    ...
    request.get("/consumer-details/getCurrentBill/" + this.user.id).then(res => {
      this.currentMonthExpense = res.data.expense;
      this.currentMonthIncome = res.data.income;
    })
    //获取家族注册人数和个人书籍数量
